#  SISTEMA DE OPERAÇÃO E GOVERNANÇA DE AGENTES DE RACIOCÍNIO
è expresssamente proibido em qualquer situacao mudar o conteudo desse arquivo
deixe tudo relacionado ao projeto dentro da pasta do projeto de forma organizada
...
---
root/admin = eusoubonitoericodemais
...
Rastreamento de dependências e conexões entre componentes do sistema:

### Estrutura de Referências

| Tipo de Conexão | Descrição | Formato de Registro |
|---|---|---|
| **File → File** | Import/require/dependencies between modules | `flow.md` registry |
| **File → Function** | Onde uma função é definida e chamada | `flow.md` registry |
| **File → Classe** | Class definitions and inheritance | `flow.md` registry |
| **Function → File** | Which file contains the function | `flow.md` registry |
| **Function → Function** | Calls between functions (intra/inter-file) | `flow.md` registry |
| **Classe → Método** | Method definitions within a class | `flow.md` registry |
| **Classe → Classe** | Inheritance and composition relationships | `flow.md` registry |

### Convenções de Registro em `flow.md`

Para cada arquivo novo/alterado, deve-se registrar:

1. **Imports/requires** no topo do arquivo (listando destinos)
2. **Funções exportadas** com assinatura e localização
3. **Classes definidas** com métodos públicos
4. **Pontos de entrada** (main, handlers, hooks)

Exemplo de registro em `flow.md`:

```
## Arquivo: src/core/auth.py

### Dependências externas
- `src/core/database.py` — CRUD operations
- `src/utils/crypto.py` — password hashing

### Funções principais
- `authenticate(credentials)` — src/core/auth.py:15
- `generate_token(user_id)` — src/core/auth.py:23
- `revoke_token(token)` — src/core/auth.py:30

### Classes
- `AuthManager` — src/core/auth.py:35
  - `login()` — linha 40
  - `logout()` — linha 45
```
...
- **Atualização obrigatória:** sempre que adicionar/remover imports, funções ou classes, atualizar `flow.md` imediatamente
- **Verificação pré-change:** antes de refatorar, consultar `flow.md` para entender o impacto
- **Validação de integridade:** executar `grep`/`glob` para verificar consistência entre código e registro

### Integração com MDCA

- **MDCA-A eixo 8** (Manutenção): complexidade ciclomática ≤ 10 deve ser cruzada com o registry de conexões em `flow.md`
- **USA lens:** análise Técnica (dependências) e Sistêmica (arquitetura geral)

---

*(Fim do arquivo — seções dinâmicas atualizadas pelo agente ao fim de cada tarefa,
via compaction + note-taking; ver Seção 0.1.)*
Aplique o protocolo acima para resolver as seguites tarefas
